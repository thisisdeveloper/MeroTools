...
import { Language, CompoundingFrequency } from '../../types';
...
export const CompoundInterestCalculator: React.FC<CompoundInterestCalculatorProps> = ({
  ...
  }, [principal, rate, years, months, frequency, deposit, depositFreq]);
...
• Time Horizon: ${result.years} Years ${result.months > 0 ? `${result.months} Months` : ''}
  ...
  return (
    <div id="compound-interest-tool" className="space-y-6">
      ...
      <div className="p-6 sm:p-7 rounded-[2rem] bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-5">
        ...
        {/* 3. Time Tenure (Years & Months) */}
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">
              {isNe ? 'अवधि (वर्ष)' : 'Tenure (Years)'}
            </label>
            <input
              id="input-years"
              type="number"
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <button
            onClick={handleCopy}
            className="self-start sm:self-center flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/20 text-white text-xs font-bold transition-all active:scale-95"
          >
