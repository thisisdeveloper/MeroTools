...
2. **Play Console store listing** — title, short/full description, category, contact email, screenshots (at least 2 phone screenshots — can generate these from the running app on request), the feature graphic (ready), content rating questionnaire, and the Data Safety form. Still no accounts, no analytics/ads SDKs, and no personal data collected — but as of the Location Reminders feature, the form must now also declare:
   ...
   - App Store (iOS) equivalent: the "App Privacy" nutrition label in App Store Connect needs the analogous **Location** entry (linked to the user via the app, not used for tracking) when that submission happens.
